...
<html>
    <head>
        <meta charset="UTF-8">
        <title>Reserva.APT - Confirmar entrega</title>
  ...
  ?>
        ...
        <div class="container">
            ...
            <form action="finalizarReserva.php" method="POST"> 
                ...
                echo "<h3><b>Confirmação de entrega</b></h3><br/>";

                $hora_atual = ((date("G") * 100) + date("i"))-500;
                $data = date("Y")."-".date("m")."-".date("d");

                $sql = mysql_query("SELECT hora_entrega FROM reserva WHERE status='entregue'");
                $sqli = mysql_query("SELECT * FROM reserva WHERE status='entregue'");

                $entregue = mysql_query("SELECT * FROM reserva WHERE status='entregue'");
                echo '<div class="table-responsive">';
                echo '<table class="table"><tr><th>Professor</th><th>Equipamento</th><th>Data</th><th>Hora Reserva</th><th>Hora Entrega</th><th>Confirmar</th></tr>';
                while ($exibe = mysql_fetch_assoc($entregue)) {
                    ...
                    if(strtotime($dataReservaConvertida)<strtotime($data)){
                    ...
                    $id = $exibe['codigo'];
                    $status  = $exibe['status'];                         
                    echo '<td> <a href="confirmar-entrega.php?id='.$id.'> <button type="button" class="btn btn-danger"><span class="glyphicon glyphicon-thumbs-down"></span></button> </a></td>';    
                        echo '</tr>';
                    ...
                    $id = $exibe['codigo'];
                    echo '<td> <a href="confirmar-entrega.php?id='.$id.'> <button type="button" class="btn btn-success"><span class="glyphicon glyphicon-thumbs-up"></span></button> </a></td>';
                    echo '</tr>';
